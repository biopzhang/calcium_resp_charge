...
if [ "$#" -lt 3 ]; then
    echo "Usage: $0 PDB_Directory Calcium_radius Perform_Minimization (Yes or No)"
...
if [ $min == "Yes" ]; then
    ...
    for i in `ls "$dir"/../PDB/loop*.pdb | head -n 1`; do
        i=`basename $i .pdb`
        ...
        rm -f New.xyz
        rm -f Min.xyz
        perl "$dir"/pdb2xyz.pl "$dir"/../PDB/${i}.pdb
    ...
    for i in `ls "$dir"/../PDB/loop*.pdb`; do
        i=`basename $i .pdb`
        loop=`echo $i | cut -f 1 -d .`
        echo working on model $i
